...
//tick
int GetTickData(THANDLE hTdb, char* szCode, char* szMarket, int nDate)
{
	//请求信息
	...
	//创建记录文档
	char FileName[64];
	sprintf(FileName,"E:\\winddownload2\\%d_%s_tickdata.csv",nDate,szCode);
	fp=fopen(FileName,"w");
...
//逐笔成交
int GetTransaction(THANDLE hTdb, char* szCode, char* szMarketKey, int nDate)
{
	//请求
	...
	FILE *fp=NULL;
	char FileName[64];
	sprintf(FileName,"E:\\winddownload\\%d_%s_transaction.csv",nDate,szCode);
	fp=fopen(FileName,"w");
...
/*		fprintf(fp,"%s,", trans.chWindCode);		
		fprintf(fp,"%d,", trans.nDate);
		fprintf(fp,"%d,", trans.nTime);
		fprintf(fp,"%c,", trans.chFunctionCode);
		fprintf(fp,"%c,", trans.chOrderKind);
		fprintf(fp,"%c,", trans.chBSFlag);
		fprintf(fp,"%d,", trans.nTradePrice);
		fprintf(fp,"%d,", trans.nTradeVolume);
		fprintf(fp,"%d,", trans.nAskOrder);
		fprintf(fp,"%d\n", trans.nBidOrder);	*/	
...
//逐笔委托
int GetOrder(THANDLE hTdb, char* szCode, char* szMarketKey, int nDate)
{
	//请求
	...
	FILE *fp=NULL;
	char FileName[64];
	sprintf(FileName,"E:\\winddownload\\%d_%s_order.csv",nDate,szCode);
	fp=fopen(FileName,"w");
...
//委托队列
int GetOrderQueue(THANDLE hTdb, char* szCode, char* szMarketKey, int nDate)
{
	//请求
	...
	FILE *fp=NULL;
	char FileName[64];
	sprintf(FileName,"E:\\winddownload\\%d_%s_orderqueue.csv",nDate,szCode);
	fp=fopen(FileName,"w");
